...
describe('IntentRouter — LLM main path', () => {
  ...
  it('returns degraded result with error when LLM classifier throws', async () => {
    ...
    // No keyword fallback — returns unknown with degraded flag
  ...
  it('returns degraded result with error when LLM classifier times out', async () => {
    vi.useFakeTimers();
...
describe('IntentRouter — no classifier (returns unknown, no keyword fallback)', () => {
  it('returns unknown with degraded flag when no classifier is provided', async () => {
    const router = new IntentRouter();
    router.registerAll([
  ...
  it('returns unknown regardless of keyword matches in input', async () => {
    const router = new IntentRouter();
  ...
  it('returns degraded with gap for any input without classifier', async () => {
    const router = new IntentRouter();
    router.register(makeSkill('slides', 'slides', ['演示文稿']));
    ...
    expect(result.gaps).toContainEqual(
      expect.objectContaining({ name: 'intent', priority: 'required' }),
    );
  });

  it('returns unknown for PPTX export intents without LLM classifier', async () => {
    const router = new IntentRouter();
    ...
    for (const rawInput of inputs) {
      ...
      // No keyword fallback — all return unknown without LLM
      expect(result.matchedSkill).toBeNull();
      expect(result.confidence).toBe(0);
      expect(result.degraded).toBe(true);
    }
