...
describe('SlidesSkill', () => {
  ...
  it('renders slides with TOC and at least three bullet points per content page', async () => {
    const skill = new SlidesSkill();
    ...
    // 5 default slides + 1 auto-inserted TOC = 6
    expect(slides.length).toBeGreaterThanOrEqual(5);

    // Content slides (non-TOC) should have at least 3 bullets
    for (const slide of slides) {
      if (slide.includes('data-slide-type="toc"')) continue;
