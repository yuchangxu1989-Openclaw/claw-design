...
describe('ExportAdapter', () => {
  ...
  it('renders structured slides with headings as bold large text in PPTX', async () => {
    const outputDir = await mkdtemp(join(tmpdir(), 'claw-design-export-'));
    const artifact = makeArtifact(`<!DOCTYPE html><html><body>
      ...
      <section class="slide" data-slide-type="content" data-slide-index="2">
        <h2>Key Points</h2>
    ...
    // Slide 1 should have the title rendered with bold formatting
    const slide1Xml = await zip.file('ppt/slides/slide1.xml')?.async('string');
    expect(slide1Xml).toContain('Main Title');
    expect(slide1Xml).toContain('Subtitle text here');

    // Slide 2 should have the heading and bullet items
    const slide2Xml = await zip.file('ppt/slides/slide2.xml')?.async('string');
    expect(slide2Xml).toContain('Key Points');
    expect(slide2Xml).toContain('First point');
    expect(slide2Xml).toContain('Second point');
    expect(slide2Xml).toContain('Third point');
  });

  it('renders comparison slides with dual-column layout in PPTX', async () => {
    const outputDir = await mkdtemp(join(tmpdir(), 'claw-design-export-'));
    const artifact = makeArtifact(`<!DOCTYPE html><html><body>
      <section class="slide" data-slide-type="comparison" data-slide-index="1">
        ...
        <div class="comparison-left">
          <h3>React</h3>
          <ul><li>Virtual DOM</li><li>JSX syntax</li></ul>
        </div>
        <div class="comparison-right">
          <h3>Vue</h3>
          <ul><li>Reactive data</li><li>Template syntax</li></ul>
        </div>
      </section>
    </body></html>`);
    ...
    expect(slide1Xml).toContain('React vs Vue');
    // Both columns' content should be present
    expect(slide1Xml).toContain('Virtual DOM');
    expect(slide1Xml).toContain('JSX syntax');
    expect(slide1Xml).toContain('Reactive data');
    expect(slide1Xml).toContain('Template syntax');
  });

  it('preserves bullet structure in PPTX for list blocks', async () => {
    const outputDir = await mkdtemp(join(tmpdir(), 'claw-design-export-'));
    const artifact = makeArtifact(`<!DOCTYPE html><html><body>
      <section class="slide" data-slide-type="content" data-slide-index="1">
        <h2>Features</h2>
        <ul>
    ...
    const slide1Xml = await zip.file('ppt/slides/slide1.xml')?.async('string');
    expect(slide1Xml).toContain('Features');
    expect(slide1Xml).toContain('Performance optimization');
    expect(slide1Xml).toContain('Developer experience');
    expect(slide1Xml).toContain('Accessibility support');
    // pptxgenjs renders bullets with buNone or buChar — verify text objects exist
    // The key assertion is that items are separate text runs, not a single blob
